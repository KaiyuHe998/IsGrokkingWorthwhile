...
def preprocess_data_bart(data):
    ...
    if type(input_text) == list:
        ...
        # Fix 1: Use squeeze(0) to safely handle length-1 targets
        ...
        # Fix 2: Use squeeze(0) to prevent 0-d tensor for single-token inputs (Identity Bridge)
        target_prefix_ids = target_prefix_ids.squeeze(0)
