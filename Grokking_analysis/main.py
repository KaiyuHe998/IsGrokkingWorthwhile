...
# Prefer the vendored simpletransformers inside this repo (portable, no absolute paths)
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
_VENDOR_ST = os.path.join(_THIS_DIR, "simpletransformers")
if _VENDOR_ST not in sys.path:
    sys.path.insert(0, _VENDOR_ST)
...
def main():
    ...
    parser.add_argument("--evaluate_train", action="store_true", help="Enable sparse checkpoint saving mode (only save weights early on)")
    ...
    # Cycle GPT2 configs
    parser.add_argument("--H_cycle", default=2, type=int, help="Number of High-level cycles (num_cycles)")
    parser.add_argument("--L_cycle", default=6, type=int, help="Number of Low-level layers (num_L_layers)")
    parser.add_argument("--hidden_size", default=256, type=int, help="Hidden size (n_embd)")
    parser.add_argument("--mlp_t", action="store_true", help="Use MLP router (False) or Attention (True)")
